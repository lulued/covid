...
# 9 July 2018 - grebe is a pod not ans, indian runner duck is domestic
# 27 july 2018 - great-horned owl (stg), Buteo buteo (common buzzard, acc), Himantopus himantopus (cha)
...
# Pod Podicipediformes  https://en.wikipedia.org/wiki/Grebe - grebe
...
# updated 19 Feb 2018
# updated 9 July 2018 - grebe is a pod not an ans
# 27 july 2018 - great-horned owl (stg), Buteo buteo (common buzzard, acc), Himantopus himantopus (cha)
birdClass	<- function( birdNames ) {
	...
	inds		<- which((birdNames=="ck") | (birdNames=="chick") | (birdNames=="ch") | (birdNames=="chiken") )
	if (length(inds) > 0) {
	...
	acc	<- c("eagle","hawk","buzzard","bussard","hooded vulture","condor","buteo buteo")

	ans	<- c("dk","duck","mallard","teal","goose","gs","swan","cygnus","wigeon",
			"munia","goldeneye","pochard","goosander",
			...
			"common porchard","anser cygnoides","anser anser")
	...
	cha	<- c("shorebird","gull","tern","turnstone","dunlin","sandpiper","sanderling",
			...
			"common redshank","black-necked stilt","himantopus himantopus")
	...
	stg	<- c("snowy owl","snowy_owl","snowyowl","long-eared owl","ural owl","eurasian eagel owl","owl owl", "great-horned owl")
	...
	orders <- list(	acc=acc,ans=ans,apo=apo,cic=cic,cor=cor,col=col,
				...
				pas=pas,pel=pel,pro=pro,pic=pic,pte=pte,pod=pod,
				rhe=rhe,str=str,sph=sph,stg=stg,psi=psi,tin=tin)
...
# updated 9 July 2018 - indian runner duck is domestic
...
# 18 nov 2015
# 9 feb 2019 - anas penelope = eurasian wigeon = long; anser anser = greylag goose = long
...
#Best regards, Rogier
	...
	# 9 feb 2019 - SJL anas penelope = eurasian wigeon; anser anser = greylag goose
	long <- c(long, c("anas penelope","anser anser"))
...
getProvinceOfChina <- function( places ) {
	...
	Jiangsu 	<- c(	"Jiawang","Nanjing","Qianzhou",
				...
				"Nantong","Siyang",
				"Huai'an","Huaian","Huaiyin")
	      # not sure about Shuanggou or Xigou

	Jiangxi 	<- c("Nanchang","Zhenjiang","Ganzhou")
	...
	Mongolia	<- c("Neimeng","Neimonggu","Neimenggu","Inner Mongolia","Inner_Mongolia")		# not completely sure about these
...
China_state_abbrev <- function( names, type="toState", 
						fname="china_state_abbreviations_oct_2018.csv",
						path="") {

	tbl 		<- as.matrix(read.table(paste(path,fname,sep=""),header=TRUE, sep=",", skip=1))
	...
	minds1	<- match(gsub("[ _]","",names), gsub(" ","",paste(tbl[,5]) ) )
	minds2	<- match(names,tbl[,2])
	minds3	<- match(names,tbl[,1])
	...
	jj3	 <- which(is.finite(minds3))
	if (length(jj3) > 0) {
		states[jj3] <- paste(tbl[minds3[jj3],k])
	}
...
Japan_state_abbrev <- function( names, type="toState", 
						fname="japan_state_abbreviations_oct_2018.csv",
						path="") {
...
# 9 Oct 2018 - Republic of Macedonia in SouthernEurope
# 16 Oct 2018 - if justContinent=TRUE then return Europe, Africa etc
unGeoScheme <- function( countries, getFullList=FALSE, justContinent=FALSE ) {
	...
	EasternAsia	<- c(		"China","Hong Kong","Macau","Macao",
	...
	SouthernEurope	<- c(	"Albania",
    					...
    					"Portugal",
    					"San Marino",
    					"Serbia","Kosovo","Serbia and Montenegro",
    					"Slovenia",
    					"Spain",
    					"The former Yugoslav Republic of Macedonia","Macedonia","Yugoslavia","Republic of Macedonia","Yugoslav SFR")
  ...
  if (getFullList) {
	...
	if (justContinent) {
		# only major continent
		mc <- c("Africa","Europe","America","Asia","Oceania","Antarctica")
		for (i in 1:length(mc)) {
			inds <- grep(mc[i],continents)
			continents[inds] <- mc[i]
		}
		return( continents )
...
# 2 april 2019
correctEasternEurope <- function( country, lat, lon ) {
	region <- unGeoScheme(country)
	inds	 <- which(lon>=120 & region=="EasternEurope")
	if (length(inds)>0) {
		region[inds] <- "EasternAsia"
	}
	inds   <- which(lon>=60 & lat>=40 & region=="EasternEurope")
	if (length(inds)>0) {
		region[inds] <- "NorthernAsia"
	}
	return( region )
}
